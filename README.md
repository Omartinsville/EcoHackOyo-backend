...
`js/forms.js` already knows how to talk to this API — JSON payloads,
honeypot field, everything. There's exactly **one line to change**, at the
top of `js/forms.js`:
...
Set it to your deployed API's base URL and every registration page picks it
up automatically — you don't need to touch the three HTML files' `action`
attributes at all (they're kept only as a Formspree fallback for anyone who
hasn't deployed the API yet; `data-endpoint="hackathon"` / `"summit"` /
`"sponsor"` on each `<form>` is what tells `forms.js` which route to call).

## Participant IDs

Every hackathon registration gets a sequential ID like `ECOHACK-2026-0001`,
generated by Postgres itself (`participant_code` in `schema.sql`, derived
from `participant_no`). It's shown on the success screen and is safe to use
at check-in / exhibition day — it can never be duplicated or skipped, even
under concurrent signups, because it's assigned from the same atomic
capacity-locking step described below. Editing an existing registration
(same email, resubmitted) keeps the original ID rather than issuing a new
one.

The Main Summit form doesn't get participant IDs — it isn't capacity-capped,
so there's no natural upper bound to format a code against. Add one later
the same way if you ever cap summit attendance too.

## WhatsApp group redirect

Set these two in `.env` (see `.env.example`):

```
WHATSAPP_HACKATHON_LINK=https://chat.whatsapp.com/xxxxxxxxxxxx
WHATSAPP_SUMMIT_LINK=https://chat.whatsapp.com/yyyyyyyyyyyy
```

On a successful hackathon or summit registration, the API returns that link
in its JSON response, and `js/forms.js` shows the success message for
~1.8 seconds, then navigates the browser straight to the WhatsApp group
(with a manual "Join WhatsApp Group Now" button underneath, in case a
browser blocks the automatic redirect). Sponsor registrations don't redirect
— sponsors aren't event attendees, so there's no group to send them to.

If a link is left blank/unset, the API just returns `whatsapp_link: null`
and the success screen shows normally without redirecting — so you can ship
this before the groups exist and switch it on later without a redeploy of
the frontend, only an env var change on the API.
